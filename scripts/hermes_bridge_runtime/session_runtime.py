...
def _update_session_mapping(
    ...
    _persistence._sync_session_mappings(
...
async def chat_stream(
    ...
    if _contracts.IN_PROCESS_STREAM_ENABLED:
        ...
        if existing is not None and not body.regenerate:
            ...
            return StreamingResponse(
                _agent_execution._busy_sse(user_id),
                media_type="text/event-stream",
                headers={
                    "Cache-Control": "no-cache",
                    "Connection": "keep-alive",
        ...
        try:
            ...
            return StreamingResponse(
                _agent_execution._sse_from_in_process(
                    user_id,
                    goal,
                    request_id=body.request_id,
...
def _require_owner_session(
    ...
    _persistence._require_internal_strict(internal_token)
...
def _owner_session_snapshot(session_key: str, sandbox: TenantHermesSandbox) -> dict[str, Any]:
    ...
    db = _agent_config._create_sandbox_session_db(sandbox)
