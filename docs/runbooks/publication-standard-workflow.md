...
时间安排：采集 01:00，收据目标 02:00，编译目标 03:00；08:05 冻结写作输入。发行以各主题 release_times 为准；写作/素材/审核应在对应 slot 前完成，延期保留原 issue_key，不借下一期掩盖。公共审核轮询每 10 分钟，公共到期发行每 5 分钟，由 store 决定哪些期次已经到期，不再为每个主题硬编码发行 cron。当前 native cron 的旧固定时刻需要激活时按配置统一重配，配置文件本身不会自动改 cron。
