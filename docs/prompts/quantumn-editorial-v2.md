...
- 连载章节 `chapter`：至少 3,000 有效汉字，明确系列和已完成/未完成学习目标；只有实际连续多章才标明章次，每日单篇不加孤立章号，不包装成整书。
...
1. 先读调度输入指定的主题、发行期次，以及配置中启用主题的既有规划与最近已发正文/目录、新增授权来源和尚未关闭的 `review-report.json`。优先修复待审稿，不为了当天日期重复首期主题。
...
9. 正文写定后只交付内容产物（`publication-content-v1`；本地 submission 仅 `title / summary / source_files / execution_files`）。返工交回新正文与指定内容证据，由调度接力调用确定性打包器，作者不执行 prepare/stage/release。平台从可信执行记录读取真实作者会话，分配 revision、issue、attempt、target 及 previous-body 绑定，素材 agent 不得替换作者身份。
10. 正文质量预检失败按明确差项返回作者，复用持久化返修流程和三次上限；字段缺失、传输失败、暂停任务属于基础设施错误，由 watchdog 保留证据并按 execution ledger 幂等恢复，不要求作者修系统字段。prepare 成功后进入独立审核。

### 独立素材任务

素材任务只消费冻结正文和来源，不改写正文、标题、摘要或原作者身份。首次成稿必须基于最终正文主旨调用本机 Codex CLI 内置 `image_generation`，实际生成两张独立构图的封面（`shelf_cover` 1440×2560、`reader_cover` 2560×1440）及固定三张正文插图（`illustration_01..03`，各 1600×900）；禁止只写提示词、使用 Python/SVG/HTML 占位、复用旧刊或通用主题图。草稿包保存 Codex 执行证据、真实模型名（工具未返回则写 `unknown`，不得猜）、完整提示词、生成时间、原始/最终 SHA-256、尺寸与逐图目视检查。缺任一文件、哈希/尺寸不符、明显畸形、乱码/水印或生成失败时稿件保持 `blocked`。内容返修默认复用同一目标已经验真的五图；只有审稿明确指出视觉缺陷或正文主旨发生变化时才重新生成。确定性打包器负责 manifest、hash、权利及系统字段；素材任务只提交真实文件和生成证据。
...
2. **非技术读者审校**：逐章问“我为什么要读？术语懂吗？原理讲了吗？示例能照做吗？每个核心步骤是否都有操作、可复制输入、预期结果与失败恢复？我还会追问什么？”提出具体缺口，不把晦涩、过短、泛泛而谈仅降为 warning。任一启用每日主题缺任一封面或任一 `illustration_01..03`、任一文件与 manifest 哈希/尺寸不符、视觉内容与正文主旨不符、缺 Codex CLI 内置 `image_generation` 证据，或选用了用户自己的产品与开发过程，一律拒绝。审稿不 stage；批准后的确定性接力必须另行回读五个私有媒体 receipt。
...
按配置 release_times 调度的确定性发行任务与有界重试只释放新门禁通过的待发稿。既有已发布正文不可变；旧待发稿必须补合同和复审。缺期仍显示缺期，不创建占位正文。
