...
class TestChatReasoningIntegration(unittest.TestCase):
    def setUp(self):
        ...
        self.state_mapping_patch = patch.object(bridge.contracts, "STATE_DB_MAPPING_FILE", Path(self.tmp_dir.name) / "state-mappings.json")
        self.state_mapping_patch.start()
        self.addCleanup(self.state_mapping_patch.stop)
