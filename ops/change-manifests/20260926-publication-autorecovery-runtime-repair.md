...
- Added a fail-closed watchdog check for tutorial READY receipts: at least one `selected_candidates` entry, verified storage/manifest, and exact `compile_verified=true` plus `wiki_compiled=true` are mandatory. A receipt claiming READY while compilation is queued now remains in prerequisite recovery; unrelated `non_blocking_unselected` failures do not block a valid selected candidate. The new watchdog tests passed `38` checks, Ruff passed, and canonical/installed runtime SHA matched `b5b67173723ec1cc606f465435b833c425306fefe025f10c074863efa108473e`.
