...
def validate_bundle(bundle: dict[str, Any], *, now: datetime | None = None) -> tuple[dict[str, Any], list[str]]:
    ...
    for item in assets:
        ...
        if (receipt["kind"] != "publication_inline_image"
                or item.get("media_type") not in _IMAGE_MEDIA_TYPES.values()
    ...
    if series["kind"] == "daily":
        ...
        if any(url not in declared_assets or declared_assets[url]["status"] != "verified" for url in image_targets):
            blocked.append("daily_public_url_image_forbidden")
...
class PublicationStore:
    ...
    def _media_asset_valid(self, db: sqlite3.Connection, asset: dict[str, Any]) -> bool:
        ...
        try:
            ...
            return False
        expected_format = {value: key for key, value in _IMAGE_MEDIA_TYPES.items()}.get(asset.get("media_type"))
        return receipt.get("kind") == expected["kind"] and actual == (expected_format, expected["size"])

    def _media_assets_valid(self, db: sqlite3.Connection, bundle: dict[str, Any]) -> bool:
        return all(self._media_asset_valid(db, item) for item in bundle.get("assets", []) if "role" in item)
    ...
    def _access_reasons(self, db: sqlite3.Connection, row: sqlite3.Row, now: datetime, vault: Path | None) -> list[str]:
        ...
        if SERIES.get(row["series_id"], {}).get("kind") == "daily" and not legacy:
            try:
                bound_urls = {asset.get("url") for asset in bundle.get("assets", [])
                              if "url" in asset and self._inline_asset_valid(db, asset)}
                if any(url not in bound_urls for url in _markdown_images(artifact_bytes.decode("utf-8"))):
                    reasons.append("daily_public_url_image_forbidden")
            except (PublicationError, UnicodeError):
                reasons.append("daily_body_media_binding_invalid")
    ...
    def prepare_editorial(self, bundle: dict[str, Any], *, review_policy=None) -> dict[str, Any]:
        ...
        try:
            ...
            if current:
                for gap in json.loads(current["gaps_json"]):
                    if gap.get("id") == "review.rejected":
                        continue
                    if gap.get("state") != "open" and current["state"] not in {"rejected", "failed"}:
                        continue
                    ...
                    # A writer or packager cannot certify a prior review gap.
                    # Keep its exact obligation open until the signed reviewer
                    # supplies a gap_resolutions finding on the new target.
                    by_id[gap["id"]] = {
                        "id": gap["id"], "question": gap["question"], "state": "open",
                        "resolution": (gap.get("resolution") if gap.get("state") == "open" and gap.get("resolution") else
                                       "Pending independent verification against: " + str(
                                           gap.get("acceptance_criterion") or gap.get("required_evidence")
                                           or gap.get("resolution") or gap["question"])),
                        "source_urls": [],
                    }
            ...
            failures = db.execute("""SELECT COUNT(*) FROM editorial_attempts
                WHERE issue_id=? AND state IN ('failed','rejected') AND revision > COALESCE((
                ...
                if not reasons:
                    dispositions = ({entry["id"]: entry for entry in review["gap_resolutions"]}
                                    if any(g.get("state") == "open" and g.get("id") != "review.rejected"
                                           for g in contract["research_gaps"]) else {})
                    verified["gaps"] = [
                        {**gap, "state": "resolved",
                         "resolution": dispositions[gap["id"]]["disposition"] + ": " + dispositions[gap["id"]]["finding"]}
                        if gap.get("state") == "open" and gap.get("id") in dispositions else gap
                        for gap in contract["research_gaps"] if gap.get("id") != "review.rejected"
                    ]
            return sorted(set(reasons))
    ...
    def status_report(self, publication_id: str | None = None, *, now: datetime | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            items = self.status(publication_id)
            actual = now or _now()
            rows = {row["edition_id"]: row for row in db.execute("SELECT * FROM editions").fetchall()}
            for item in items:
